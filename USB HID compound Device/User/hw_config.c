...
/* Extern variables ----------------------------------------------------------*/
extern __IO uint8_t KeyBoardSendComplete, ControlerSendComplete;

/* Private function prototypes -----------------------------------------------*/
static void IntToUnicode (uint32_t value , uint8_t *pbuf , uint8_t len);
/* Private functions ---------------------------------------------------------*/

static __IO uint8_t KeyBoard_Buffer[15] = {}, Controller_Buffer[2];
...
/**
  * Function Name : ControllerState
  * Description   : 更新按键信息，具体缓冲区与按键的对应关系参考报告描述符和HID Usage Table(P89)，按键更新可以进一步封装
  * Input         : None.
  * Output        : None.
  * Return value  : The direction value.
  */
uint8_t ControllerState(void) {
	uint8_t state = 0, i;
	
	MatrixKeyBoard_Scan();
	for (i = 0; i < 2; i++) {
		if (Controller_Buffer[i] ^ MatrixKeyBoard_Code[i + 15]) {
			Controller_Buffer[i] = MatrixKeyBoard_Code[i + 15];
			state = 1;
		}
	}
...
/**
  * Function Name : Controler_Send.
  * Description   : prepares buffer to be sent containing KeyBoard event infos.
  * Input         : Keys: keys received from terminal.
  * Output        : None.
  * Return value  : None.
  */
void Controler_Send(void) {
	/* Reset the control token to inform upper layer that a transfer is ongoing */
	ControlerSendComplete = 0;

	/* Copy mouse position info in ENDP2 Tx Packet Memory Area*/
	USB_SIL_Write(EP2_IN, (uint8_t*)Controller_Buffer, 2);
