...
void OLED_W_DC(uint8_t BitValue) {
	...
	GPIO_WriteBit(GPIOA, DC_Pin, (BitAction)BitValue);
}
...
void OLED_W_CS(uint8_t BitValue) {
	...
	GPIO_WriteBit(GPIOA, CS_Pin, (BitAction)BitValue);
}
...
void DMA1_Channel3_IRQHandler(void) {
	if (DMA_GetITStatus(DMA1_IT_TC3) == SET) {
		...
		switch (DMA1_Channal3_StateFlag) {
		case 0x00:
		case 0x01:
		case 0x02:
		case 0x03:
		...
		case 0x05:
		case 0x06:
			DMA1_Channal3_StateFlag++;
			DMA_Cmd(DMA1_Channel3, DISABLE);
			
			OLED_W_DC(0);
		
			SPI_I2S_SendData(SPI1, 0xB0 | DMA1_Channal3_StateFlag); //设置页位置
			...
			DMA1_Channel3->CMAR = (uint32_t)OLED_DisplayBuf[DMA1_Channal3_StateFlag];
...
void OLED_DrawBKNet() {
	uint8_t j;
	for (j = 0; j < 128; j++) {
		OLED_DisplayBuf[0][j] = OLED_DisplayBuf[3][j] = 0;
		if (j & 0x02) {
			OLED_DisplayBuf[1][j] = OLED_DisplayBuf[4][j] = 0x08;
			OLED_DisplayBuf[2][j] = OLED_DisplayBuf[5][j] = 0x80;
		}
		else {
			OLED_DisplayBuf[1][j] = OLED_DisplayBuf[4][j] = 0x00;
			OLED_DisplayBuf[2][j] = OLED_DisplayBuf[5][j] = 0x00;
		}
	}
}
