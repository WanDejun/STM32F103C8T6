...
/**
  ...
  */
uint8_t OLED_DisplayBuf[12 * 2][128];
...
void OLED_MenuAdd(char* str) {
	...
	for (i = 0; str[i] != '\0'; i++) {
		for (j = 0; j < 8; j++) {
			OLED_DisplayBuf[MenuItemCnt << 1][i << 3 | j] = OLED_F8x16[str[i] - ' '][j];
			OLED_DisplayBuf[MenuItemCnt << 1 | 1][i << 3 | j] = OLED_F8x16[str[i] - ' '][j | 0x08];
		}
	}
...
void OLED_MenuClear(void) {
	uint8_t i, j;
	for (i = 0; i < 128; i++) {
		for (j = 0; j < MenuItemCnt << 1; j++) {
			OLED_DisplayBuf[j][i] = 0;
		}
	}
}
