...
static void OLED_WriteRollData(uint8_t *Data, uint8_t Count) {
	uint8_t i = p_front;
...
void OLED_RollUpdate(void) {
	...
	for (j = 0; j < 6; j ++) {
		...
		OLED_SetCursor(j, 0);
		/*连续写入128个数据，将显存数组的数据写入到OLED硬件*/
		OLED_WriteRollData(OLED_DisplayBuf[j], 128);
	}
	for (j = 6; j < 8; j ++) {
		/*设置光标位置为每一页的第一列*/
		OLED_SetCursor(j, 0);
...
void OLED_RollAddDate(uint8_t data) { // 0 ~ 47
	if (data >= 48) return;
	uint8_t i;
	for (i = 0; i < 6; i++) {
		OLED_DisplayBuf[i][p_front] = 0;
	}
	OLED_DisplayBuf[data >> 3][p_front] = 1 << (data & 0x07);
	p_front = (p_front + 1) & 127; //对128取模
}
